...
let scrollSpeed = 0; // 現在のスクロール速度
let lastTimestamp = null;

// requestAnimationFrameで常にスクロール
function forceScroll(timestamp) {
    if (lastTimestamp !== null) {
        ...
        // 減速させたい場合はここで減衰
        // scrollSpeed *= 0.98; // 必要なら慣性スクロール風に
    }
...
scrollSliderRight.addEventListener('input', () => {
    ...
    scrollSliderLeft.value = scrollSliderRight.value; // 左右同期
});
...
window.addEventListener('wheel', (e) => {
    scrollSpeed = e.deltaY * speedScale;
    e.preventDefault(); // ネイティブスクロールを止める場合
}, { passive: false });
